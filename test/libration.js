...
(function() {
    ...
    function calculateC80andB72(B53) {
        ...
        const C55 = B55 ** 2;
        const D55 = B55 ** 3;
        const B75 = ((134.963 + 477199 * B55 + 0.008997 * C55 + D55 / 69700) % 360 + 360) % 360;
        const C75 = B75 * Math.PI / 180;

        const B76 = ((297.85 + 445267 * B55 - 0.00163 * C55 + D55 / 545900) % 360 + 360) % 360;
        const B77 = 2 * B76;
        const C77 = B77 * Math.PI / 180;

        const B72 = ((93.2721 + 483202 * B55 - 0.003403 * C55 - D55 / 3526000) % 360 + 360) % 360;
    ...
    function calculateB82(B53) {
        ...
        const B59 = ((357.529 + 35999 * B55 - 0.0001536 * C55 + D55 / 24490000) % 360 + 360) % 360;
        const C59 = B59 * Math.PI / 180;

        const B73 = ((218.316 + 481268 * B55) % 360 + 360) % 360;
    ...
    function topocentricLibration(year, month, day, hour, minute, latitudeDeg, longitudeDeg) {
        ...
        const Lo = ((B94 - B72 + 180) % 360 + 360) % 360 - 180;
        ...
        return [Bo + B139, Lo + B138];
    }

    function geocentricLibration(year, month, day, hour, minute) {
        const B53 = calculateB53(year, month, day, hour, minute);
        const B74 = calculateB74(B53);
        const B78 = calculateB78(B53);
        const [C80, B72] = calculateC80andB72(B53);
        ...
        const B92 = Math.sin(C91) * Math.cos(C80Rad) * Math.cos(C90) - Math.sin(C80Rad) * Math.sin(C90);
        const B93 = Math.cos(C91) * Math.cos(C80Rad);
        const C94 = Math.atan2(B92, B93);
        const B94 = ((C94 * 180 / Math.PI) % 360 + 360) % 360;
        const Lo = ((B94 - B72 + 180) % 360 + 360) % 360 - 180;

        const B96 = -Math.sin(C91) * Math.cos(C80Rad) * Math.sin(C90) - Math.sin(C80Rad) * Math.cos(C90);
        const C97 = Math.asin(B96);
        const Bo = C97 * 180 / Math.PI;
    ...
    window.EMEastroLib = {
        ...
        geocentricLibration,
        calculateLST: calculateB53,
        calculateLSTradians: calculateC54LSTradians,
        calculateB74,
        calculateB78,
        calculateC80andB72,
        calculateB82,
    ...
    console.log("EMEastroLib © 2025 Max Romensky, SA5IKN. All rights reserved.");
})();
